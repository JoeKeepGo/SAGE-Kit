...
- Stop if new authority, false-green, approval-boundary, security-boundary,
  required-check-failure, source-authority, or evidence-integrity risk appears:
  `yes`
...
Capability Routing:
- Required skills:
- Selected external workflow bundle skills, if available:
- external workflow bundle boundary, if used:
...
- Full review required because semantics, permission, source authority,
  ...
  required-check meaning, or required evidence changed: `<yes/no>`
- Evidence to inspect:
- Acceptance criteria:
