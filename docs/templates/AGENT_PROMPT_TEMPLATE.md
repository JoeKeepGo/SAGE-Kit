...
- Apply `docs/agent/CAPABILITY_ADAPTERS.md#sage-adp-007` before generic routing;
  ...
  launch packet.
- Codex GPT-5.6 launch guard when active: external workflow bundle is
  `DISABLED_BY_RUNTIME_POLICY`; controllers and descendants must not read,
  invoke, route to, reference, or delegate to external workflow bundle, and
  `implicit workflow bootstrap` remains disabled. Use the canonical model-native fallback.
