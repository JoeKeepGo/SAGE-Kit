...
## Structured records Policy

State whether this milestone adopts structured task/evidence records.
...
- task/evidence record paths;
- required evidence levels by task class;
- project-native check and the gate it supports;
- shared-file or shared-resource conflict policy;
...
- Structured-record contract or project policy reference:
...
If external workflow bundle is available, list the specific skills that may be used for
...
- blocking gates are `PASS` or explicitly `WAIVED` by the named Waiver
  ...
  authority;
- Structured records acceptance-gate check passes when the profile is active and a
  task, phase, or milestone acceptance gate is being advanced;
- project-selected structured-record check is not marked `n/a` for an acceptance gate unless the
  specific task is recorded as not an acceptance gate;
- State Truth Reconciliation is `PASS` when Structured records is active;
