...
inputs.
Corrective re-review scope is canonical at
`docs/agent/EXECUTION_ECONOMY.md#sage-loop-010`;
...
- Skills used:
- external workflow bundle skills used:
- external workflow bundle boundary violations:
...
- Structured records active: `<yes/no>`
- Task/evidence records reviewed:
- Project-native check:
- Check result:
- L0-L4 evidence gaps:
- Shared-file or resource conflicts:
...
- P2 findings that block acceptance because they affect authority, false-green,
  approval gates, security boundaries, required-check requirements,
...
- New authority, false-green, approval-gate, security, required-check,
...
- Semantic, permission, source authority, information architecture, contract,
  runtime, security, approval gate, or required-check meaning changed: `<yes/no>`
