...
- external workflow bundle boundary notes:
...
- Acceptance-gate evidence: `<command/result, or n/a only when not an acceptance gate>`
...
When Structured records is active, a packet that asks Project Manager to advance a
task, phase, or milestone acceptance gate must include acceptance-gate check
