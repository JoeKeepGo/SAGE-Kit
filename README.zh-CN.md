...
- 强制拆细 milestone 的规划规则，让 phase 可评审、可测试、边界明确。
...
1. 填写 `PROJECT_PROFILE.md`。
2. 编写或适配 `TECHNICAL_DESIGN.md`。
3. 定义 `QUALITY_GATES.md` 和 `APPROVAL_GATES.md`。
4. 添加 `ACTIVE_CONTEXT.md` 和 `DOC_ROUTING.md`。
5. 用 `00-entry-gate.md` 创建第一个 milestone。
6. 将 milestone 拆成边界明确、可评审、可测试的 phases。
7. 通过保留的 phase docs 和 completion reports 执行每个 phase。
8. 当 Phase 内存在安全并行 lane 时使用 Wave Execution。
9. 当大型 milestone 需要 Project Manager、Coder、Final Review 控制器交接时使用 Session Orchestration。
10. 只有 Project Manager 授权时，才使用 Worktree Isolation 创建独立 phase、lane 或 review 工作区。
11. 在 `MILESTONE_LEDGER.md` 中维护 milestone 状态。
12. Milestone 关闭时，写入 `MILESTONE_CLOSEOUT.md` 作为紧凑的历史结果索引。
