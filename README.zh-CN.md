...
SAGE-Kit 还包含一个小型 Python CLI 原型。这个 CLI 不是 AI coding
agent，也不替代内置 skill。它的作用是给人、agent 和 CI 一个稳定的方式来检查治理文档。

无需安装即可使用跨平台 Python module 入口：
...
如果希望在 Windows、macOS 和 Linux 上使用统一的本地命令，可以从仓库安装 package
entrypoint：
...
`sagekit doctor` 是只读诊断命令，用来判断当前目录更像 SAGE-Kit 源仓库还是已采用
SAGE-Kit 的项目，并检查 package entrypoint 和内置 Task Dispatch validator 是否可用。

`sagekit check` 是面向 gate 的 validator。它会检查 required/recommended docs、
`ACTIVE_CONTEXT.md`、`DOC_ROUTING.md`、phase docs、completion reports、adapter
evidence 和 Task Dispatch records。只要存在 `FAIL` finding 就退出 `1`；只有
`PASS` 或 `WARN` 时退出 `0`。

`sagekit init` 用来为目标项目创建 SAGE-Kit 治理文档。它支持
`--mode light`、`--mode standard` 和 `--mode heavy`；`--dry-run` 只打印计划写入，
不修改文件；`--force` 只覆盖当前 mode 选择的目标文件。它会拒绝在 SAGE-Kit 源仓库中运行。
...
```mermaid
flowchart TD
  A["任务或想法"] --> B["读取 ACTIVE_CONTEXT.md"]
  B --> C["读取 DOC_ROUTING.md"]
  C --> D["只读取路由到的 milestone、phase、gate 或 packet 文档"]
  D --> E["选择 Governance Level: Light / Standard / Heavy"]
  E --> F["选择 Permission Mode: read-only / write / corrective / environment / submit"]
  F --> G["写明文件边界、gates、tests、smoke、evidence 和 stop conditions"]
  G --> H{"是否触发 closed gate 或 scope expansion?"}
  H -- "是" --> I["停止，交给 Project Manager 或用户决策"]
  H -- "否" --> J{"是否需要专业能力?"}
  J -- "是" --> K["使用 Capability Adapter: detect, authorize, bound, invoke, capture, map, fallback"]
  J -- "否" --> L["使用 SAGE-Kit 原生 phase 或 review 路径"]
  K --> L
  L --> M{"工作类型"}
  M -- "实现" --> N["执行已批准的 phase、lane 或 corrective task"]
  M -- "评审" --> O["独立验证并分类 findings"]
  N --> P["运行 tests、runtime smoke，并收集 evidence"]
  P --> O
  O --> Q{"是否需要 correction?"}
  Q -- "是" --> R["Corrective packet、PM decision request、blocker 或 waiver path"]
  R --> N
  Q -- "否" --> S["Completion report、ledger 和 memory maintenance"]
  S --> T["Handoff、accept decision 或 next prompt"]
```
