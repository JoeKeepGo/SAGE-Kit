...
- External Capability Boundary，用来明确 SPEC-Kit 是 governance 和
...
SPEC-Kit 不是 skill library。参见 `docs/SPEC_CORE.md#external-capability-boundary`：外部 capability 只能在 SPEC-Kit 的 authorization、scope、ownership、evidence、lock 和 gate 边界内执行。Superpowers 是可用时的参考集成，不是必需依赖，也不是 SPEC-Kit 要复制的内容。
...
flowchart TD
  ...
  J -- "否" --> L["使用 SPEC-Kit 原生 phase 或 review 路径"]
  K --> L
  L --> M{"工作类型"}
