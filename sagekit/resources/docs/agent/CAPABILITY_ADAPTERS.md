...
When work involves UI, styling, React, Next.js, design-system components,
browser behavior, accessibility, responsive layout, or visual QA, detect and
select an available frontend or browser-testing adapter when it is useful for
the active boundary. If no adapter is available, authorized, or useful, record
the fallback and continue through the SAGE-Kit-native path when safe.

The selected adapter must stay inside the active SAGE-Kit boundary and should
produce:
...
The selected adapter must stop before:
