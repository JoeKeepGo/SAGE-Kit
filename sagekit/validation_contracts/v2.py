...
def validate_records(
    ...
    *,
    ...
    from ..task_dispatch_validator import validate_records as validate_current_records
    ...
    for label, record in (("task", task), ("evidence", evidence)):
        ...
        if not isinstance(metadata, dict):
            errors.append(f"{label} validation_contract is missing or invalid")
            continue
    ...
    errors.extend(
        validate_current_records(task, evidence, schema_dir(), gate_ready=gate_ready)
    )
    return errors
